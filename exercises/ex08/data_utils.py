...
def convert(values: dict[str, list[str]]) -> dict[str, list[int]]:
    """Changes strings to integers for dicts with lists of strings that are numbers."""
...
def average(values: dict[str, list[int]]) -> dict[str, float]:
    """Finds the average value of a list in a dictionary and creates a new dictionary with those values."""
    average_values: dict[str, float] = {}
    for key in values:
        average_values[key] = ((sum(values[key])) / len(values[key]))
