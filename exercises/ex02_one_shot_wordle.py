...
guess = input(str(f"What is your {len(secret_word)}-letter guess? "))
...
while len(guess) != len(secret_word):
    guess = input(str(f"That in not {len(secret_word)} letters! Try again: "))
    ...
    if guess[i] == secret_word[i]:
        ...
        while not test and b < len(secret_word):  # b allowes the computer to test each letter of your guess against every letter in the secret word
            if guess[i] == secret_word[b]:
                ...
                test: bool = True  # stops the while loop if letter exists in word
                ...
                if b == len(secret_word) - 1:  # program will only reach this step if the letter is not in the secret word
                    ...
                    b = 0
                    test: bool = True  # exits the loop if the letter is not in the secret word
                else:
                    b += 1
                
    i += 1  # ensures the loop is progressing forward
    test: bool = False  # resets the bool value of test so loop can be run for the next letter
print(result)
